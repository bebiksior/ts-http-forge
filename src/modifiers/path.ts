...
export function setPath(
  ...
  if (path.includes(" ")) path = path.replaceAll(" ", "%20");

  return modify(lines, path);
}

function modify(lines: HTTPRequestLines, path: string): HTTPRequestLines {
...
function replacePathInLine(requestLine: string, newPath: string): string {
  let methodEnd = 0;
  ...
  const remainingPart =
    pathEnd < requestLine.length ? requestLine.substring(pathEnd) : "";
  ...
  if (questionMarkIndex !== -1 && hashIndex !== -1) {
    queryAndFragment = pathAndQuery.substring(
  ...
  return (
    method + spacesAfterMethod + newPath + queryAndFragment + remainingPart
  );
}
